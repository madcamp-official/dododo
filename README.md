...
동일한 Ground Truth Fixture를 프로젝트 최초 스캐폴드 커밋(`3afec0b`)과 최종 구현에서
비교했다. 최초 버전에는 Context 파이프라인이 없어 기대 항목 2건과 병합 쌍 1건이 모두
누락되었고, 최종 버전은 실제 파이프라인 실행 결과를 평가기로 채점했다.

| 지표 | 개선 전 | 개선 후 | 변화 |
|---|---:|---:|---:|
| Context 생성 F1 | 0% | 100% | +100%p |
| 다중 출처 Context 병합 정확도 | 0% | 100% | +100%p |
| False Merge | 0건 | 0건 | 0건 |

Context 생성 F1은 제목·종류·마감·Evidence 수가 모두 일치할 때만 정답으로 인정하며,
누락과 잘못된 생성을 함께 반영한다. 병합 정확도는 학교 사이트와 이메일에 중복된 같은
공지가 하나의 Context로 합쳐졌는지 측정하고, 서로 다른 정보를 합친 False Merge를 별도로
센다. 최종 결과는 `node --test tests/evaluation.test.ts`로 재현할 수 있으며 평가 테스트
11개가 모두 통과했다.

현재 수치는 Ground Truth 2건과 다중 출처 병합 쌍 1건을 대상으로 한 MVP 대표 시나리오
결과다. 따라서 다양한 학교·LMS 전체에 대한 일반화 성능이 아니라, 정의한 수직 흐름이
처음부터 끝까지 정확하게 동작하는지를 보여 주는 지표로 해석한다.
