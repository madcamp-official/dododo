...
test("한 번도 연 적 없으면 createWindow로 새 창을 만들고 초기 route로 보여준다", () => {
  ...
  coordinator.open({ view: "today" }, { x: 10, y: 20 }, (route) => {
    createCount += 1;
    receivedRoute = route;
  ...
  assert.deepEqual(window.calls, ["position:10,20", "show"]);
});
...
test("이미 열려 있으면 새로 만들지 않고 navigate 후 show·focus한다", () => {
  ...
  coordinator.open({ view: "today" }, { x: 10, y: 20 }, createWindow);
  coordinator.open({ view: "detail", itemId: "task-1" }, { x: 30, y: 40 }, createWindow);

  assert.equal(createCount, 1);
  assert.deepEqual(window.calls, ["position:10,20", "show", "position:30,40", "navigate", "show", "focus"]);
...
test("닫힌 뒤에는 다시 open()하면 새 창을 만든다", () => {
  ...
  coordinator.open({ view: "today" }, { x: 10, y: 20 }, createWindow);
  first.destroy();
  coordinator.open({ view: "calendar" }, { x: 30, y: 40 }, createWindow);

  assert.equal(createCount, 2);
  assert.deepEqual(second.calls, ["position:30,40", "show"]);
});
...
test("closed 이벤트 없이 isDestroyed()만 true여도 새로 만든다(방어적 확인)", () => {
  ...
  coordinator.open({ view: "inbox" }, { x: 30, y: 40 }, createWindow);
