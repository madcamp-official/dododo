...
test("SQLite: retry는 attempts를 늘리고 pending으로 되돌리며 true를 반환한다", async () => {
  const queue = createRepository();
  await queue.enqueue({ id: "extract_facts:raw-1", type: "extract_facts", inputRef: "raw-1", now: NOW });
  const claimed = await queue.claimNext(["extract_facts"], NOW, 60_000);
...
test("SQLite: complete 이후에는 다시 claim되지 않고 true를 반환한다", async () => {
  const queue = createRepository();
  await queue.enqueue({ id: "extract_facts:raw-1", type: "extract_facts", inputRef: "raw-1", now: NOW });
  const claimed = await queue.claimNext(["extract_facts"], NOW, 60_000);
...
test("SQLite: deadLetter로 보낸 작업은 listDeadLetters에 나오고 다시 claim되지 않으며 true를 반환한다", async () => {
  const queue = createRepository();
  await queue.enqueue({ id: "extract_facts:raw-1", type: "extract_facts", inputRef: "raw-1", now: NOW });
  const claimed = await queue.claimNext(["extract_facts"], NOW, 60_000);
