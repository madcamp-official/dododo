...
// docs/frontend-plan.md 6.8.2: 창 제어용 단방향 채널이라 Result<T> 데이터 IPC
// (desktopApi)와 분리한다. openPanel은 캐릭터 Renderer가 결과 패널을 열 때,
// onPanelNavigate는 패널 Renderer 자신이 route/itemId 변경을 받을 때 쓴다 — 둘 다
// 같은 preload를 캐릭터·패널 창이 함께 재사용하므로 한 global에 같이 둔다.
