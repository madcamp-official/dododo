...
// 관리한다. 창이 3개라 각자의 storage 컨텍스트를 신경 쓰는 것보다 Main 하나가 소유하는
// 편이 안전하다는 문서 판단을 그대로 따른다. 실제 userData 경로는 register.ts가
// electron의 app.getPath로 주입하고, 이 파일은 경로를 매개변수로 받아 electron 없이도
// 테스트할 수 있게 한다.
