...
| 추가(`add`) | 예, 날짜·시간·내용(장소는 자유 입력, `metadata.location`으로 저장) | 있음 — 폼 입력으로 구조화된 값을 그대로 저장(파싱 불필요, 더 단순함)하되, 폼 위에 자연어 입력칸(선택)을 두어 CLI와 같은 `parseScheduleIntentWithLlmFallback`으로 폼 필드를 미리 채운다. 저장은 항상 폼 제출 한 경로뿐이라 모호한 일정도 사용자가 채워진 값을 보고 확인·수정한 뒤에만 저장된다(별도 확정 경로 없음). 알림설정 입력은 리마인더 오프셋 기능(2.4) 선행 필요 |
...
모든 NotificationEvent는 도착 시 캐릭터 말풍선으로 순서대로 표시한다. `ask:ask`의
성공 답변은 `answer` 즉시 이벤트로도 전달해 캐릭터가 답변을 말풍선으로 발화한다.
