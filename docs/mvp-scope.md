...
3. 추천 이유가 포함된 Opportunity가 생성된다.
4. 사용자가 지원 준비를 선택하면 Task와 마감 Event가 생성된다.
5. LMS 공지에서 과제와 시험을 추출한다.
6. 수정 공지로 기존 마감이 갱신되고 이전 값이 보존된다.
7. `today` 명령이 오늘 할 일과 가까운 마감을 보여준다.
8. 자연어 약속을 입력하면 확인 후 Calendar에 저장한다.
9. 현재 화면 분석이 관련 Task를 찾아 구체적인 조언을 제공한다.
10. 모든 결과에서 원본 근거를 확인할 수 있다.
